...
public class Migration_8_9 extends Migration {
    ...
    public void migrate(SupportSQLiteDatabase supportSQLiteDatabase) {
        ...
        supportSQLiteDatabase.execSQL(
                "CREATE TABLE `new_minerva_courses` (`_id` TEXT, `code` TEXT, `title` TEXT, `description` TEXT, `tutor` TEXT, `academic_year` INTEGER NOT NULL, `ordering` INTEGER NOT NULL, PRIMARY KEY(`_id`))"
        );
        ...
        // Create the new table.
        supportSQLiteDatabase.execSQL(
                "CREATE TABLE `new_minerva_calendar` (`_id` INTEGER NOT NULL, `title` TEXT, `content` TEXT, `start_date` INTEGER, `end_date` INTEGER, `location` TEXT, `type` TEXT, `last_edit_user` TEXT, `last_edit` INTEGER, `last_edit_type` TEXT, `course` TEXT, `calendar_id` INTEGER NOT NULL, PRIMARY KEY(`_id`), FOREIGN KEY(`course`) REFERENCES `minerva_courses`(`_id`) ON UPDATE NO ACTION ON DELETE NO ACTION )"
        );
        // Copy the data.
        supportSQLiteDatabase.execSQL(
                "INSERT INTO new_minerva_calendar (_id, title, content, start_date, end_date, location, type, last_edit_user, last_edit, last_edit_type, course, calendar_id) SELECT _id, title, content, start_date, end_date, location, type, last_edit_user, last_edit, last_edit_type, course, calendar_id FROM minerva_agenda"
        );
        // Drop the old table.
